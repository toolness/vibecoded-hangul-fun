...
export function decompose_hangul_syllable_to_jamos(
  ...
  const medialCodepointIdx = Math.floor((baseCodepoint - (initialCodepointIdx * 588)) / 28);
  const finalCodepointIdx = baseCodepoint - (initialCodepointIdx * 588) - (medialCodepointIdx * 28);
  ...
  const maybeFinalCh = finalCodepointIdx === 0 ? undefined : String.fromCharCode(finalCodepoint);
...
function hangul_syllable_to_jamos(ch: string): string | null {
  ...
  return maybeFinal ? `${initial}${medial}${maybeFinal}` : `${initial}${medial}`;
}
